...
static const oid_t m_ereader_oid		= { { 1, 3, 6, 1, 10, 1, 1				},  7, 8  };
...
int mib_build(void)
{
	...
	if(!mib_alloc_entry(&m_ereader_oid,  1, 0, BER_TYPE_COUNTER) ||
	   !mib_alloc_entry(&m_ereader_oid,  2, 0, BER_TYPE_GAUGE))
		return -1;
...
int mib_update(int full)
{
	...
	if (full) {
		get_ereaderinfo(&u.ereaderinfo);
		if (update_int(&m_ereader_oid,  1, 0, &pos, u.ereaderinfo.documentCount)   == -1 ||
		    update_int(&m_ereader_oid,  2, 0, &pos, u.ereaderinfo.batteryLevel)    == -1)
			return -1;
	}
